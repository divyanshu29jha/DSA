...
// NOTE: We need to do push and pop operations in O(1) time. So, push and pop from start (head) of the linked list, not from tail!
// If we push at the tail, then pop operation is not possible in O(1) time, as we will have to traverse the LL to reach the previous node of tail.
...
public:
  ...
  void push(int data) {
    ...
    if (head == NULL) {
      ...
      // and then update the head to the new node, effectively pushing the new
      // node onto the stack
  ...
  void pop() {
    ...
    free(temp); // Free the memory occupied by the temporary node (the previous
                // top of the stack)
  }
  ...
  int getTop() {
    ...
    // Otherwise, return the data stored in the head node, which is the top of
    // the stack
    return head->data;
  }
};
